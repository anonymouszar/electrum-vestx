...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-vestx-macos
	cd /tmp/electrum-vestx-macos
...
rm -rf /tmp/electrum-vestx-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-vestx-macos/image/
cp -r $1 /tmp/electrum-vestx-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Vestx-Electrum_uncompressed.dmg \
    /tmp/electrum-vestx-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum_uncompressed.dmg electrum-vestx-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
sha256sum electrum-vestx-$VERSION.dmg
