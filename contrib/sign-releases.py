...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.vestx.org -keyalg RSA -keysize 2048 \
        -validity 10000
...
SDIST_NAME_PATTERN = re.compile('^Vestx-Electrum-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Vestx-Electrum-{version}'
PPA_SOURCE_NAME = 'electrum-vestx'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_VESTX-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Vestx-Electrum-{version}-release.apk'
